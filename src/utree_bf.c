...
static void
utree_inner_destroy(utree_t* node)
{
  free(node->next->next);
  free(node->next);
  free(node);
...
static void
utree_tip_destroy(utree_t* node)
{
  if (node->label)
    free(node);
}
...
static void
utree_link(utree_t* a, utree_t* b)
{
  /*
...
static void
utree_edgesplit(utree_t* a, utree_t* b, utree_t* c)
{
  /*
                *                                      *
                |                                      |
...
static void
utree_exhaust_recursive(utree_t** edge_list,
                        utree_t** inner_nodes,
                        utree_t** tip_nodes,
                        int depth)
{
  ...
  if (*(tip_nodes + 1)) {
    ...
    while (edge_list[i]) {
      ...
      utree_exhaust_recursive(
        edge_list, inner_nodes + 1, tip_nodes + 1, depth + 1);
...
static void
utree_exhaust(unsigned int tips_count, char** tip_labels)
{
  unsigned int i;
  ...
  */
  ...
  utree_t** inner_node_list =
    (utree_t**)calloc(tips_count - 2, sizeof(utree_t*));
