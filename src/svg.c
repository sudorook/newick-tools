...
static coord_t*
create_coord(double x, double y)
{
...
static void
svg_line(double x1, double y1, double x2, double y2, double stroke_width)
{
  fprintf(svg_fp,
...
static void
svg_circle(double cx, double cy, double r)
{
  fprintf(svg_fp,
...
svg_utree_plot(utree_t* node)
{
  ...
  if (parent) {
    ...
    if (!node->next) {
      ...
      svg_line(x, ly, x, ry, stroke_width);

      svg_circle(x, y, opt_svg_inner_radius);
    }
    /* horizontal line */
    svg_line(px, y, x, y, stroke_width);
...
static void
utree_scaler_init(utree_t* root, int tip_count)
{
  double len = 0;
  double label_len;
  int i;
...
static void
rtree_scaler_init(rtree_t* root)
{
  double len = 0;
  double label_len;
  unsigned int i;
...
void
svg_utree_init(utree_t* root, int tip_count)
{
  long svg_height;
