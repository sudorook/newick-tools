...
static int
cb_asc(const void* va, const void* vb)
{
  double a = *(double*)va;
  double b = *(double*)vb;
...
static double
create_waiting_time(double s, double t)
{
  ...
  double denum =
    opt_birthrate - opt_deathrate * e - opt_birthrate * (1 - e) * s;
...
static void
set_branchlength(rtree_t* node, double parent_age)
{
  double* node_ageptr;
