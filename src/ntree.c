...
static void
ntree_query_tipnodes_recursive(ntree_t* node, ntree_t** node_list, int* index)
{
  int i;
  if (!node)
    return;
...
ntree_t**
ntree_query_tipnodes(ntree_t* node, int* count)
{
  int i;
  ntree_t** node_list;
...
static void
ntree_node_count_recursive(ntree_t* root,
                           int* inner_count,
                           int* tip_count,
...
resolve_random(ntree_t* node)
{
  ...
  if (node->children_count == 0) {
    ...
    rtree_t** children =
      (rtree_t**)xmalloc(node->children_count * sizeof(rtree_t*));
...
static rtree_t*
ntree_to_rtree_recursive(ntree_t** nodes, int count)
{
  rtree_t* rtree;
  ...
  if (count == 1) {
    ...
    if (node->children == NULL) {
      ...
      rtree->right =
        ntree_to_rtree_recursive(node->children + 1, node->children_count - 1);
...
ntree_to_rtree(ntree_t* root)
{
  ...
  rtree->right =
    ntree_to_rtree_recursive(root->children + 1, root->children_count - 1);
